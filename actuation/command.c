...
void servo(struct throttle_steer *ptr){
	...
	printf("%f",ptr->steer); // prints steering input stored in shared memory object                                                                                   	
	...
	//converts input in range [-1 1] to values for servo controller
	float target = SERVO_LEFT + ((ptr->steer - START) / (END - START))*SPAN;
	...
	float target2 = THROTTLE_LEFT + ((ptr->throttle - START) / (END - START))*SPAN_2;
...
int main(){
	...
	//I moved all the code from the steering into its own function, don't call it yet before finding throttle ranges
